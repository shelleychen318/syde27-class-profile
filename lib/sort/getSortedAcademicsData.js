...
import { createBoxPlotData, getMaxRespondents } from "./sortFunctions";
...
export async function getSortedAcademicsData() {
  ...
  for (var object in data) {
    ...
    if (
      data[object].favCourse != "" &&
      favCourse.hasOwnProperty(data[object].favCourse)
    ) {
      favCourse[data[object].favCourse][0]++;
    }

    if (
      data[object].leastFavCourse != "" &&
      leastFavCourse.hasOwnProperty(data[object].leastFavCourse)
    ) {
      leastFavCourse[data[object].leastFavCourse][0]++;
    }
  }

  const favCourses = Object.keys(favCourse);
  const favCourseValues = Object.values(favCourse);
  const leastFavCourseValues = Object.values(leastFavCourse);
  const favCourseRespondents = getMaxRespondents(favCourse, leastFavCourse);
