...
export async function getSortedDemographicsData() {
  ...
  for (var object in data) {
    ...
    for (let i = 0; i < data[object].ethnicity.split(", ").length; i++) {
      if (data[object].ethnicity.split(", ")[i] in ethnicUnique) { ethnicUnique[data[object].ethnicity.split(", ")[i]] += 1 }
      else if (data[object].ethnicity.split(", ")[i] != '') { ethnicUnique[data[object].ethnicity.split(", ")[i]] = 1 }
    }
    ...
    for (let i = 0; i < data[object].religion.split(", ").length; i++) {
      if (data[object].religion.split(", ")[i] in religionUnique) { religionUnique[data[object].religion.split(", ")[i]] += 1 }
      else if (data[object].religion.split(", ")[i] != '') { religionUnique[data[object].religion.split(", ")[i]] = 1 }
    }
  ...
  const ethnicValues = Object.values(ethnicUnique);
